...
public class JdFlowLogService {
     ...
     * Dùng cho convertToDTO() trong JobDescriptionService
     * và fetchInbox() trong JdFlowService
     * ==========================================
     */
    @Getter
        ...
        if (ups != null && !ups.isEmpty()) {
            ...
            if (department == null) {
                department = ups.stream()
                        ...
                        .map(up -> up.getSectionJobTitle().getSection().getDepartment().getName())
                        .findFirst().orElse(null);
            }
            ...
            positionCode = ups.stream()
                    .filter(up -> "DEPARTMENT".equals(up.getSource())
                            && up.getDepartmentJobTitle() != null
                            && up.getDepartmentJobTitle().getJobTitle() != null
                            && up.getDepartmentJobTitle().getJobTitle().getPositionLevel() != null)
                    .map(up -> up.getDepartmentJobTitle().getJobTitle().getPositionLevel().getCode())
                    .findFirst().orElse(null);
            ...
            if (positionCode == null) {
                positionCode = ups.stream()
                        .filter(up -> "SECTION".equals(up.getSource())
                                && up.getSectionJobTitle() != null
                                && up.getSectionJobTitle().getJobTitle() != null
                                && up.getSectionJobTitle().getJobTitle().getPositionLevel() != null)
                        .map(up -> up.getSectionJobTitle().getJobTitle().getPositionLevel().getCode())
                        .findFirst().orElse(null);
            }
            ...
            if (positionCode == null) {
                positionCode = ups.stream()
                        .filter(up -> "COMPANY".equals(up.getSource())
                                && up.getCompanyJobTitle() != null
                                && up.getCompanyJobTitle().getJobTitle() != null
                                && up.getCompanyJobTitle().getJobTitle().getPositionLevel() != null)
                        .map(up -> up.getCompanyJobTitle().getJobTitle().getPositionLevel().getCode())
                        .findFirst().orElse(null);
            }
        }

        return new RejectInfo(log.getComment(), rejector.getName(),
                position, department, positionCode); // ← THÊM positionCode
    }
